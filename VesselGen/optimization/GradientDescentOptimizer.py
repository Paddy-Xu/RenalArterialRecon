    ...
    def cost(self, testMedian):
        ...
        if self.cost_mode == 'MC':
            ...
            for i in range(self.num_points):
                surface_term = self.vsize * 2 * np.linalg.norm(testMedian - self.dataPoints[i]) * self.testRadius[i] if \
                    self.include_surface_area else 0
                temp += self.vsize * np.linalg.norm(testMedian - self.dataPoints[i]) * (self.testRadius[i] ** 2) + \
                        surface_term
            ...
            if self.include_surface_area:
                temp1 = np.array([self.vsize * np.linalg.norm(testMedian - self.dataPoints[i]) * self.testRadius[i] ** 2 +
            ...
            temp2 = np.array([self.testRadius[i] ** 4 / (self.vsize * np.linalg.norm(testMedian - self.dataPoints[i]) +
                                                         1e-10)
                              for i in range(1, len(self.dataPoints))])
            temp2 = 1 / np.sum(temp2) + self.vsize * np.linalg.norm(testMedian - self.dataPoints[0]
                                                                    ) / self.testRadius[0] ** 4
            temp2 *= self.w2
